...
class NasdaqSharadarProvider(MarketDataProvider):
    """Licensed Nasdaq Data Link / Sharadar profile.

    Sharadar is not CRSP: it can provide active and delisted ticker coverage,
    EOD prices, metadata, and corporate actions, but it does not provide CRSP
    DLRET. Downstream audit/reporting must disclose that distinction.
    """

    provider_name = "nasdaq_sharadar"
    ...
    def load_daily_returns(self) -> list[CRSPDailyReturnRecord]:
        return load_csv_records(
            self.config.data_provider.sharadar_prices_file,
            CRSPDailyReturnRecord,
        )
